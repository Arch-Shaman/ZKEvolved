...
local spDestroyProjectile = Spring.DestroyProjectile
...
function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions)
	...
	if cmdID == CMD.INSERT and cmdParams[2] == CMD.ATTACK and #cmdParams == 4 then
		...
		if spValidUnitID(cmdParams[4]) then
			local x,y,z = spGetUnitPosition(cmdParams[4])
			spGiveOrderToUnit(unitID, CMD.INSERT, {cmdParams[1], cmdParams[2], cmdParams[3], x, y, z}, cmdOptions.coded)
			return false
		end
	end
	...
	if cmdID == CMD.ATTACK and #cmdParams == 1 then
		...
		if Spring.ValidUnitID(cmdParams[1]) then
			...
			spGiveOrderToUnit(unitID, CMD.ATTACK, { x, y, z}, cmdOptions.coded)
			return false
		end
	end
...
function gadget:ProjectileCreated(proid, owner, wd)
	if interceptorID[wd] then
		interceptor[proid] = true
	end
end
...
function gadget:AllowWeaponInterceptTarget(interceptorUnitID, interceptorWeaponNum, targetProjectileID)
	...
	local ux, _, uz = spGetUnitPosition(interceptorUnitID)
