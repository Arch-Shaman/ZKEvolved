...
--  author:  Dave Rodgers
--
--  modified by jK and quantum. Optimized by _Shaman.
--
...
function includeZIPFirst(filename, envTable)
  if (strfind(filename, '.h.lua', 1, true)) then
    filename = 'Headers/' .. filename
  end
  return vfsInclude(LUAUI_DIRNAME .. filename, envTable, VFS.ZIP_FIRST)
end

WG = {}
Spring.Utilities = {}
vfsInclude("LuaRules/Utilities/tablefunctions.lua")
vfsInclude("LuaRules/Utilities/versionCompare.lua")
local reverseCompat = not Spring.Utilities.IsCurrentVersionNewerThan(100, 0)

vfsInclude("LuaRules/Utilities/function_override.lua")
...
local detailLevel = spGetConfigInt("widgetDetailLevel", 3)
...
function string:split(delimiter) -- optimized by removing table.insert and replacing it with #result+1.
	local result = {}
	local from = 1
	local delim_from, delim_to = strfind(self, delimiter, from)
	while delim_from do
		result[#result+1] = strsub(self, from, delim_from-1)
		from = delim_to + 1
		delim_from,delim_to = strfind(self,delimiter,from)
	end
...
function widgetHandler:LoadOrderList()
  ...
  if (chunk == nil) then
	...
	local detailLevel = spGetConfigInt("widgetDetailLevel", 2)
...
function widgetHandler:SaveConfigData()
  ...
  for _,w in ipairs(self.widgets) do
    if (w.GetConfigData) then
	  ...
	  if not ok then spLog(HANDLER_BASENAME, LOG.ERROR, "Failed to GetConfigData from: " .. w.whInfo.name.." ("..err..")") end 
    end
  end
...
function widgetHandler:Initialize()
  ...
  if customkeys["ignored"] then
    if strfind(customkeys["ignored"],",") then
      ...
      spEcho("Setting Serverside ignorelist: " .. newignorelist)
      for ignoree in strgmatch(newignorelist,"%S+") do
        ignorelist.ignorees[ignoree] = true
        ignorelist.count = ignorelist.count + 1
      end
      newignorelist = nil
    elseif strlen(customkeys["ignored"]) > 1 then
  ...
  local autoModWidgets = spGetConfigInt('LuaAutoModWidgets', 1)
  self.autoModWidgets = (autoModWidgets ~= 0)

  -- create the "LuaUI/Config" directory
  spCreateDir(LUAUI_DIRNAME .. 'Config')
  ...
  for _,w in ipairs(unsortedWidgets) do
    if (w.whInfo.api) then
      ...
      spEcho(strformat("Loaded API widget:  %-18s  <%s>", name, basename))
    end
  end
  ...
  for _,w in ipairs(unsortedWidgets) do
    if (not w.whInfo.api) then
      ...
      local name = w.whInfo.name
      local basename = w.whInfo.basename
      spEcho(strformat("Loaded widget:  %-18s  <%s>", name, basename))
    end
  end
...
function widgetHandler:LoadWidget(filename, _VFSMODE)
  ...
  if (chunk == nil) then
    spLog(HANDLER_BASENAME, LOG.ERROR, 'Failed to load: ' .. basename .. '  (' .. err .. ')')
    return nil
  end
  ...
  if (knownInfo) then
    if (knownInfo.active) then
      spLog(HANDLER_BASENAME, LOG.ERROR, 'Failed to load: ' .. basename .. '  (duplicate name)')
      return nil
    end
  else
  ...
  if (widget.GetInfo == nil) then
    spLog(HANDLER_BASENAME, LOG.ERROR, 'Failed to load: ' .. basename .. '  (no GetInfo() call)')
    return nil
  end
...
function widgetHandler:NewWidget()
  ...
  wh.AddLayoutCommand = function (_, cmd)
    if (self.inCommandsChanged) then
      ...
      spLog(HANDLER_BASENAME, LOG.ERROR, "AddLayoutCommand() can only be used in CommandsChanged()")
    end
  end
...
local function HandleError(widget, funcName, status, ...)
  ...
  if (funcName ~= 'Shutdown') then
    ...
    spLog(HANDLER_BASENAME, LOG.ERROR, 'Error in Shutdown()')
  end
  local name = widget.whInfo.name
  local error_message = select(1,...)
  spLog(HANDLER_BASENAME, LOG.ERROR, 'Error in ' .. funcName ..'(): ' .. tostring(error_message))
  spLog(HANDLER_BASENAME, LOG.ERROR, 'Removed widget: ' .. name)
  return nil
end
...
local function SafeWrapFuncGL(func, funcName)
  ...
  return function(w, ...)
    ...
    if (r[1]) then
      ...
      local name = w.whInfo.name
      spLog(HANDLER_BASENAME, LOG.ERROR, 'Error in ' .. funcName ..'(): ' .. tostring(r[2]))
      spLog(HANDLER_BASENAME, LOG.ERROR, 'Removed widget: ' .. name)
      return nil
    end
  end
...
local function SafeWrapFunc(func, funcName)
  if (not SAFEDRAW) then
    ...
    if (strsub(funcName, 1, 4) ~= 'Draw') then
      return SafeWrapFuncNoGL(func, funcName)
    else
...
local function SafeWrapWidget(widget)
  ...
  elseif (SAFEWRAP == 1) then
    if (widget.GetInfo and widget.GetInfo().unsafe) then
      spLog(HANDLER_BASENAME, LOG.ERROR, 'LuaUI: loaded unsafe widget: ' .. widget.whInfo.name)
      return
    end
  end
...
function widgetHandler:RemoveWidget(widget)
  ...
  if (widget.GetConfigData) then
	...
	if not ok then spLog(HANDLER_BASENAME, LOG.ERROR, "Failed to GetConfigData: " .. name.." ("..err..")") end 
  end
  self.knownWidgets[name].active = false
...
function widgetHandler:UpdateWidgetCallIn(name, w)
  ...
  if (ciList) then
    ...
    spLog(HANDLER_BASENAME, LOG.ERROR, 'UpdateWidgetCallIn: bad name: ' .. name)
  end
end
...
function widgetHandler:RemoveWidgetCallIn(name, w)
  ...
  if (ciList) then
    ...
    spLog(HANDLER_BASENAME, LOG.ERROR, 'RemoveWidgetCallIn: bad name: ' .. name)
  end
end
...
function widgetHandler:EnableWidget(name)
  ...
  if (not ki) then
    spLog(HANDLER_BASENAME, LOG.ERROR, "EnableWidget(), could not find widget: " .. tostring(name))
    return false
  end
  if (not ki.active) then
    spEcho('Loading:  '..ki.filename)
...
function widgetHandler:DisableWidget(name)
  local ki = self.knownWidgets[name]
  if (not ki) then
    spLog(HANDLER_BASENAME, LOG.ERROR, "DisableWidget(), could not find widget: " .. tostring(name))
    return false
  end
  if (ki.active) then
    local w = self:FindWidget(name)
    if (not w) then return false end
    spEcho('Removed:  '..ki.filename)
...
function widgetHandler:ToggleWidget(name)
  local ki = self.knownWidgets[name]
  if (not ki) then
    spLog(HANDLER_BASENAME, LOG.ERROR, "ToggleWidget(), could not find widget: " .. tostring(name))
    return
  end
  if (ki.active) then
...
function widgetHandler:Shutdown()
  spEcho("Start widgetHandler:Shutdown")
  self:SaveOrderList()
  spEcho("Shutdown - SaveOrderList Complete")
  self:SaveConfigData()
  spEcho("Shutdown - SaveConfigData Complete")
  for _,w in ipairs(self.ShutdownList) do
    local name = w.whInfo.name or "UNKNOWN NAME"
	spEcho("Shutdown Widget - " .. name)
	w:Shutdown()
  end
  spEcho("End widgetHandler:Shutdown")
  
  return
end

function widgetHandler:Update()
  local deltaTime = spGetLastUpdateSeconds()  
...
function widgetHandler:ConfigureLayout(command)
  ...
  elseif (command == 'selector') then
    ...
    self:RaiseWidget(sw)
    return true
  elseif (strfind(command, 'togglewidget') == 1) then
    self:ToggleWidget(strsub(command, 14))
    return true
  elseif (strfind(command, 'enablewidget') == 1) then
    self:EnableWidget(strsub(command, 14))
    return true
  elseif (strfind(command, 'disablewidget') == 1) then
    self:DisableWidget(strsub(command, 15))
    return true
  end
...
local MUTE_SPECTATORS = spGetModOptions().mutespec or 'autodetect'
local MUTE_LOBBY = spGetModOptions().mutelobby or 'autodetect'
...
do
	...
	for _, teamID in ipairs(teams) do
		...
		if ((teamLuaAI == nil or teamLuaAI == "") and teamID ~= gaiaTeam) then
			local _,_,_,ai,side,ally = spGetTeamInfo(teamID)
	...
	if MUTE_LOBBY then
		...
		for i = 1, #playerList do
			...
			if playerID then
				local name, _, spectating = spGetPlayerInfo(playerID)
...
function widgetHandler:AddConsoleLine(msg, priority)
  ...
  elseif StringStarts(msg, transmitMagic) then -- receiving from the lobby
    if StringStarts(msg, voiceMagic) then
      local tableString = strsub(msg, strlen(voiceMagic) + 1) -- strip the magic string
	...
	if newMsg.msgtype ~= 'other' and newMsg.msgtype ~= 'autohost' and newMsg.msgtype ~= 'userinfo' and newMsg.msgtype ~= 'game_message' then 
		if MUTE_SPECTATORS and newMsg.msgtype == 'spec_to_everyone' then
			local spectating = select(1, spGetSpectatingState())
			if not spectating then
				return
			end
		...
		-- IGNORE FEATURE--
        	if ignorelist.ignorees[select(1,spGetPlayerInfo(playerID_msg))] then
			return
        	end
	end
	...
	if MUTE_LOBBY and newMsg.msgtype == 'autohost' then
		...
		if (not spectating) and newMsg.argument then
			...
			if strsub(newMsg.argument, 1, 1) == "<" then
				local endChar = strfind(newMsg.argument, ">")
				if endChar then
					local name = strsub(newMsg.argument, 2, endChar-1)
					if playerNameToID[name] then
						local spectating = select(3, spGetPlayerInfo(playerNameToID[name]))
						if spectating then
	...
	if newMsg.msgtype == 'autohost' and newMsg.argument and strsub(newMsg.argument, 1, 1) == "<" then
		local endChar = strfind(newMsg.argument, ">")
		if endChar then
			local name = strsub(newMsg.argument, 2, endChar-1)
	...
	if newMsg.msgtype == 'userinfo' and newMsg.argument then
		...
		local info = {
			...
			icon = list[3], 
			badges = list[4],
			admin = list[5] and strlower(list[5]) == 'true', 
			clan = list[6], 
			faction = list[7], 
			country = list[8], 
		}
  ...
  for _,w in ipairs(self.GameStartList) do
	...
	for _, teamID in ipairs(spGetTeamList()) do
		...
		if ((teamLuaAI == nil or teamLuaAI == "") and teamID ~= gaiaTeam) then
			local _,_,_,ai,side,ally = spGetTeamInfo(teamID)
			if (not ai) then 
				for _, pid in ipairs(spGetPlayerList(teamID)) do
					local name, active, spec = spGetPlayerInfo(pid)
					if active and not spec then plist = plist .. "," .. name end
				end
			end	
		end
	end
	spSendCommands("wbynum 255 SPRINGIE:stats,plist".. plist)
  return
end
...
function widgetHandler:MapDrawCmd(playerID, cmdType, px, py, pz, ...)
  ...
  if ignorelist.ignorees[select(1,spGetPlayerInfo(playerID))] or (customkeys and customkeys.muted) then
    return true
  end
