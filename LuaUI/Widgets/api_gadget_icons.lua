...
--local iconsSetUnitIcon = WG.icons.SetUnitIcon -- This cannot be optimized cuz api_gadget_icons loads before unit_icons
--local iconsSetOrder = WG.icons.SetOrder -- Seriously, fix me.
--local iconsSetDisplay = WG.icons.SetDisplay -- Sadness of Sad.
--local iconsSetPulse = WG.icons.SetPulse -- Made me cry. 0/10
...
function widget:Initialize()
	...
	for i=1, #AllUnits do -- shiny new code
		local unitDefID = spGetUnitDefID(AllUnits[i])
		widget:UnitCreated(AllUnits[i], unitDefID, myTeamID)
	end
	--for _, unitID in ipairs(Spring.GetAllUnits()) do -- ugly old code. ipairs is bad and should feel bad.
