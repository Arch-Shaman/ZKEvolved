...
local spSendLuaUIMsg = SendLuaUIMsg
...
local scrW, scrH = spGetViewGeometry()
...
local function DoBuildingsClash(buildData1, buildData2)
	...
	return abs(buildData1[2] - buildData2[2]) < w1 + w2 and
	       abs(buildData1[4] - buildData2[4]) < h1 + h2
end
...
function widget:Initialize()
	if (spGetGameFrame() > 0) then		-- Don't run if game has already started
		spEcho("Game already started or Start Position is randomized. Removed: Initial Queue ZK") --added this message because widget removed message might not appear (make debugging harder)
		widgetHandler:RemoveWidget(self)
		return
	end
	if spGetModOptions().singleplayercampaignbattleid then -- Don't run in campaign battles.
		widgetHandler:RemoveWidget(self)
		return
	end
...
function widget:DrawScreen()
	glPushMatrix()
	glTranslate(scrW*0.4, scrH*0.35, 0)
	local num = #buildQueue
	if num > 0 then	
		--glText(string.format(queueTimeFormat, mCost, buildTime), 0, 0, fontSize, 'cdo')
...
local function DrawWorldFunc()
	...
	if startChosen then
		...
		local rot = (abs(Game.mapSizeX/2 - sx) > abs(Game.mapSizeZ/2 - sz))
...
local function explode(div,str) --copied from gui_epicmenu.lua
  ...
  for st,sp in function() return string.find(str,div,pos,true) end do -- This looks like awful code.
...
function widget:GameFrame(n)
	...
	if (n == 4) then
		--spEcho("> Starting unit never spawned !")
		widgetHandler:RemoveWidget(self)
		return
	end
	...
	for u = 1, #units do
		local uID = units[u]
		if GetUnitCanCompleteQueue(uID) then --spGetUnitDefID(uID) == sDefID then
	...
	if tasker then
		...
		for b = 1, #buildQueue do
			local buildData = buildQueue[b]
			spGiveOrderToUnit(tasker, -buildData[1], {buildData[2], buildData[3], buildData[4], buildData[5]}, CMD.OPT_SHIFT)
		end
...
function widget:CommandsChanged()
	...
	for i=1, #buildOptions do
		...
		if not spGetGameRulesParam("disabled_unit_" .. unitName) then
...
function widget:CommandNotify(cmdID, cmdParams, cmdOptions)
	...
	if Spring.TestBuildOrder(selDefID, bx, by, bz, buildFacing) ~= 0 then
		if isMex[selDefID] and WG.metalSpots then
			...
			by = max(0, spGetGroundHeight(bx, bz))
		end
		local buildData = {selDefID, bx, by, bz, buildFacing}
		...
		if msg then
			...
			spSendLuaUIMsg(msg,'s') --need 2 msg because since Spring 97 LuaUIMsg without parameter is send info to EVERYONE (including enemy)
		end
		if msg2 then
			spSendLuaUIMsg(msg2,'a')
			spSendLuaUIMsg(msg2,'s')
		end
...
function widget:TextCommand(cmd)
	...
	if m then
		...
		spEcho("Buildings set to face " .. ({"South", "East", "North", "West"})[1 + newFacing])
		return true
	end
